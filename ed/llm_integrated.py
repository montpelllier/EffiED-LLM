from ed.init import *
from ed.prompt_templates import gen_fewshot_prompt, gen_rule_prompt
from ed.util import *
from evaluate.evaluation import evaluate_prediction, evaluate_column_predictions
...
gemma3_12b_it = 'gemma-3-12b-it'
gemma3_4b_it = 'gemma-3-4b-it'
gemma3n_e4b_it = 'gemma-3n-e4b-it'

gemini2_flash = "google/gemini-2.0-flash-exp:free"
kimi2 = "moonshotai/kimi-k2:free"
...
url, key = google_url, google_key
# url, key = openrouter_url, openrouter_key

llm = OllamaLLM(model_name=model_name)
# llm = OpenAILLM(model_name=model_name, url=url, key=key, measure=True)
...
for col in data_error.columns:
    ...
    sys_prompt = """
You are an error detection assistant. Respond ONLY in valid JSON.
"""
    ...
    for idx, label in zip(col_repre, sample_labels):
        ...
        if label != real_label:
            logger.info(
                f"predicted label *{label}* does not match actual label *{real_label}* for index {idx}: {data_error.iloc[idx][col]}")

    prediction = propagate_labels(col_cluster, col_repre, sample_labels)
    pred_df[col] = prediction

if isinstance(llm, OpenAILLM) and llm.measure:
    logger.info(f"Total tokens used: {llm.total_tokens}")
    logger.info(f"Total time taken: {llm.total_time:.2f} seconds")

evaluate_prediction(err_labels, pred_df, logger)
